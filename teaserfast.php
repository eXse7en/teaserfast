...
class Bot extends Functions{
	...
	public function __construct(){
		...
		$r = $this->Dashboard();
		if(!$r['Username'])$this->removeConfig("cookie");
	...
	public function Dashboard(){
		$r = $this->curl(host,$this->headers())[1];
	...
	public function Claim(){
		while(true){
			...
			foreach($ids as $a => $idc){
				...
				$data = "dt=".$code;
				$r = json_decode($this->curl(host.'captcha-start/',$this->headers(1),$data)[1],1);
				if(!$r['success'])break;
				while(true){
					$data = "yd=$id&hd=$hd&rc=$rc";
					$r = json_decode($this->curl(host.'captcha-youtube/',$this->headers(1),$data)[1],1);
					if(!$r['success'])break;
					...
					$r = json_decode($this->curl(host.'check-youtube/',$this->headers(1),$data)[1],1);
					if($r['captcha']){
